        ...
        {categories.map(cat => (
          <section key={cat.id} style={{ marginBottom: isMobile ? 36 : 56 }}>
            ...
            {/* Сетка товаров:
                ...
                планшет — 3 колонки
                десктоп — 5 колонок */}
            <div className="client-view-grid" style={{ display: viewMode === "grid" ? "grid" : "none", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(auto-fill, minmax(180px, 1fr))", gap: isMobile ? 12 : 24 }}>
              {cat.products.map(p => <ClientCard key={p.id} product={p} isMobile={isMobile} />)}
            </div>
            <div className="client-view-list" style={{ display: viewMode === "list" ? "flex" : "none", flexDirection: "column", gap: 1 }}>
