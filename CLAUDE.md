...
### Сессия 2026-06-04 — фикс отображения суммы по позиции в клиентской версии

**Исправлено** (коммит `cb36006`):

1. **`ClientPage.jsx` — `ClientCard` и `ClientListRow`**: вместо цены за единицу теперь показывается итог по позиции (`qty × price`)
   - Было: `× 3` / `50 000 ₽` — непонятно откуда берётся итог
   - Стало: `3 × 50 000 ₽` / **`150 000 ₽`** — сумма крупно, расшифровка мелко
   - Когда qty = 1 — просто цена, без лишних подсказок
